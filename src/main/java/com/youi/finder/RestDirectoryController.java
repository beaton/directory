...
public class RestDirectoryController {
	...
	/**
	 * Alexa JSON request to get usage data from Google Calendar Service.
	 * This method calls the Google calendar usage service to return
	 * meeting room usage detail.
	 */
    ...
    public ResponseEntity getUsageData(@RequestBody InvocationRequest aRequest) {
		logger.info("POST getUsageData request: " + aRequest.toString());
		logger.info("Alexa request version: " + aRequest.getVersion());
	...
	/**
	 * Health monitor for I'm alive ping from the Load Balancer.
	 * Will return 200 HTTP response status code by default, feel
	 * free to add custom code to verify the server is healthy and where
	 * it is not, return 4xx or 5xx as appropriate.
	 */
	...
	/**
	 * Sample endpoint to verify outward calls are functional.
	 * Makes a call to the gturnquist-quoters URL to get a quote about Spring boot for fun.
	 */
